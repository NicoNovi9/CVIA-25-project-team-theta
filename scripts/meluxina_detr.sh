...
#SBATCH --output=meluxina_detr.out
#SBATCH --error=meluxina_detr.err
...
echo "Starting DeepSpeed training script on 2 nodes, 4 GPUs per node (8 in total)..."
srun python -u src/train_detr_ddp.py \
    --deepspeed --deepspeed_config scripts/ds_config_detr.json
