"""
DeepSpeed distributed training script for SegFormer Optimized segmentation model.
Uses YOLO-guided bounding box extraction for focused segmentation.

The key optimization:
- YOLO detects spacecraft bounding boxes
- Segmentation is performed only on the bbox region (padded/resized to 512x512)
- Most bboxes are smaller than 512x512, so they're padded (preserving resolution)
- Larger bboxes are downsampled
- All pixels outside bbox are classified as background

Usage:
    # Single node (4 GPUs)
    deepspeed --num_gpus=4 train_segformer_optimized.py --config config_segformer_optimized.yaml
    
    # Multi-node via SLURM
    srun python -u train_segformer_optimized.py --config config_segformer_optimized.yaml
"""
...
from segformer_optimized_model import SegFormerOptimizedSegmentor, list_available_variants
...
def merge_config_with_args(config: dict, args: argparse.Namespace) -> dict:
    ...
    # Note: YOLO model path is no longer used - detections are precomputed
    if args.debug:
...
def train_model(
    ...
    """Training loop for SegFormer Optimized with DeepSpeed."""
    training_cfg = config['training']
    ...
    learning_rates = []
    gpu_stats_all = []
    bbox_stats = {'padded': 0, 'downsampled': 0}
    ...
    if global_rank == 0:
        print("\n" + "=" * 70)
        print("TRAINING STARTED (SegFormer Optimized - YOLO-guided Segmentation)")
        print("=" * 70)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx == 0 and epoch == 0 and global_rank == 0:
                ...
                print(f"Sample bbox sizes: {[(m['bbox_width'], m['bbox_height']) for m in metadatas[:3]]}")
                print("------------------\n")
        ...
        bbox_stats['padded'] += epoch_padded
        bbox_stats['downsampled'] += epoch_downsampled
        ...
        if global_rank == 0:
            ...
            print("-" * 70)
            print("BBOX STATISTICS (this epoch):")
            print(f"  Padded (< 512): {epoch_padded} | Downsampled (>= 512): {epoch_downsampled}")
            print("-" * 70)
        ...
        if run_validation:
            ...
            if improved:
                ...
                if global_rank == 0:
                    ...
                    base_model.save_pretrained(os.path.join(save_path, "segformer_optimized_best"))
                    print(f"✓ Best model saved to {save_path}/segformer_optimized_best")
            else:
                no_improve_epochs += 1
                if global_rank == 0:
        ...
        if (epoch + 1) % checkpoint_every == 0 and global_rank == 0:
            os.makedirs(save_path, exist_ok=True)
            base_model = model_engine.module if hasattr(model_engine, 'module') else model_engine
            base_model.save_pretrained(os.path.join(save_path, f"segformer_optimized_epoch_{epoch+1}"))
    ...
    if global_rank == 0:
        os.makedirs(save_path, exist_ok=True)
        base_model = model_engine.module if hasattr(model_engine, 'module') else model_engine
        base_model.save_pretrained(os.path.join(save_path, "segformer_optimized_final"))
        print(f"\n✅ Final model saved to {save_path}/segformer_optimized_final")
        ...
        total_samples = bbox_stats['padded'] + bbox_stats['downsampled']
        print(f"\n📊 BBox Statistics (total training):")
        print(f"   Padded (preserved resolution): {bbox_stats['padded']} ({100*bbox_stats['padded']/total_samples:.1f}%)")
        print(f"   Downsampled: {bbox_stats['downsampled']} ({100*bbox_stats['downsampled']/total_samples:.1f}%)")
...
def parse_args():
    parser = argparse.ArgumentParser(description="SegFormer Optimized Training with DeepSpeed")
    ...
    # Note: YOLO model path is no longer used - detections are precomputed in CSV
...
def main():
    ...
    if global_rank == 0:
        print("\n" + "=" * 70)
        print("SEGFORMER OPTIMIZED TRAINING (Precomputed BBoxes)")
        print("=" * 70)
        ...
        print(f"Train detection CSV: {data_cfg['train_detection_csv']}")
        print(f"Val detection CSV: {data_cfg['val_detection_csv']}")
        print("=" * 70)
    ...
    # ==========================================================================
    if global_rank == 0:
        print("\nLoading SegFormer Optimized model...")
    ...
    # ==========================================================================
    if global_rank == 0:
        print("\nPreparing datasets with precomputed bbox extraction...")
    
    data_root = data_cfg['data_root']
    target_size = training_cfg['image_size']
    bbox_cfg = config.get('bbox', {})
    
    train_dataset = SparkSegmentationOptimizedDataset(
        ...
        split="train",
        detection_csv=data_cfg['train_detection_csv'],
    ...
    val_dataset = SparkSegmentationOptimizedDataset(
        csv_path=os.path.join(data_root, data_cfg['val_csv']),
        image_root=os.path.join(data_root, data_cfg['image_root']),
        mask_root=os.path.join(data_root, data_cfg['mask_root']),
        split="val",
        detection_csv=data_cfg['val_detection_csv'],
        target_size=target_size,
        bbox_expansion=bbox_cfg.get('expansion', 1.1),
        fallback_mode=bbox_cfg.get('fallback_mode', 'full_image'),
        augment=False
...
def save_training_report(config, training_stats, world_size, save_path):
    ...
    bbox_stats = training_stats.get('bbox_stats', {'padded': 0, 'downsampled': 0})
    total_bbox = bbox_stats['padded'] + bbox_stats['downsampled']
    
    summary = {
        'timestamp': training_stats['training_start_time'].strftime('%Y%m%d_%H%M%S'),
        'model': f"SegFormerOptimized-{config['model']['variant']}",
        ...
        'detection_csv': data_cfg.get('train_detection_csv', 'N/A'),
        'bbox_expansion': bbox_cfg.get('expansion', 1.1),
        'bbox_padded_pct': 100 * bbox_stats['padded'] / total_bbox if total_bbox > 0 else 0,
