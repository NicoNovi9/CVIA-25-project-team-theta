"""
Evaluation script for UNet3+ segmentation model.
...
def evaluate_model(model, dataloader, device, save_examples=False, save_path=None, use_fp16=True):
    """
    Evaluate UNet3+ model on a dataset.
...
def main(args):
    ...
    model = UNet3PlusSegmentor.load_pretrained(args.model_path, device=device)
    ...
    if args.output_path:
        ...
        with open(report_path, 'w') as f:
            f.write("=" * 70 + "\n")
            f.write("UNET3+ 3-CLASS SEGMENTATION EVALUATION REPORT\n")
            f.write("=" * 70 + "\n\n")
            
            f.write("MODEL INFORMATION:\n")
            f.write(f"  Backbone: {model.backbone_name}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate UNet3+ segmentation model")
    
    parser.add_argument("--model_path", type=str, default="model_weights_unet3plus/unet3plus_best",
    ...
    parser.add_argument("--output_path", type=str, default="evaluation_results_unet3plus",
