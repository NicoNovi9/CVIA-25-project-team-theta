"""
Multi-GPU Inference script for UNet3+ segmentation model.
...
from unet3plus_model import UNet3PlusSegmentor


def parse_args():
    parser = argparse.ArgumentParser(description="UNet3+ Segmentation Inference")
    
    # Model path
    parser.add_argument("--model_path", type=str, required=True,
                        help="Path to UNet3+ model directory")
    ...
    # Output
    parser.add_argument("--output_dir", type=str, default="submission_output_unet3plus",
    ...
    parser.add_argument("--save_visualizations", action="store_true",
                        help="Save RGB visualization images")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Segmentation")):
            ...
            for j in range(len(filenames)):
                ...
                if save_visualizations:
                    vis_path = os.path.join(vis_dir, filename.replace('.jpg', '_pred.png').replace('.png', '_pred.png'))
                    rgb_resized.save(vis_path)
...
def main():
    ...
    print("=" * 70)
    print("UNET3+ SEGMENTATION INFERENCE")
    print("=" * 70)
    ...
    segmentation_model = UNet3PlusSegmentor.load_pretrained(
        args.model_path,
        device=device
    )
    
    print(f"  Backbone: {segmentation_model.backbone_name}")
    print(f"  Classes: {segmentation_model.n_classes}")
    ...
    print(f"  cd {args.output_dir} && zip -r submission.zip *.npz")
